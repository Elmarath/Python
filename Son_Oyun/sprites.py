...
class Knight(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        self.knight_img = pg.image.load(
            path.join(img_folder, KNIGHT_IMG)).convert_alpha()
        self.knight_img_0 = self.knight_img.copy()

        self.image = self.knight_img.copy()
        self.rect = self.image.get_rect()
        self.rect.center = (x, y)
        self.hit_rect = MOB_HIT_RECT.copy()
        self.hit_rect.center = self.rect.center
    ...
    def update(self):
        ...
        if target_dist.length_squared() < DETECT_RADIUS**2:
            if random() < 0.002:
                choice(self.game.zombie_moan_sounds).play()
            self.rot = target_dist.angle_to(vec(1, 0))
            self.image = pg.transform.rotate(self.knight_img_0, self.rot-90)
            self.rect.center = self.pos
            self.acc = vec(1, 0).rotate(-self.rot)
            self.avoid_mobs()
            self.acc.scale_to_length(self.speed)
...
class Wizard(pg.sprite.Sprite):
    ...
    def __init__(self, game, x, y):
        ...
        self.wizard_img = pg.image.load(
            path.join(img_folder, WIZARD_IMG)).convert_alpha()
        self.wizard_img_0 = self.wizard_img.copy()

        self.image = self.wizard_img.copy()
        self.rect = self.image.get_rect()
        self.rect.center = (x, y)
        self.hit_rect = MOB_HIT_RECT.copy()
        self.hit_rect.center = self.rect.center
    ...
    def update(self):
        ...
        if target_dist.length_squared() < (3*DETECT_RADIUS / 2)**2:
            if random() < 0.002:
                choice(self.game.zombie_moan_sounds).play()
            self.rot = target_dist.angle_to(vec(1, 0))
            self.image = pg.transform.rotate(self.wizard_img_0, self.rot-90)
            self.rect.center = self.pos
            self.acc = vec(1, 0).rotate(-self.rot)
            # self.avoid_mobs()
            self.avoid_player()
...
class Archer(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        self.archer_shooting_img = pg.image.load(
            path.join(img_folder, ARCHER_SHOOTING_IMG)).convert_alpha()
        self.archer_shooting_img_0 = self.archer_shooting_img.copy()

        self.image = self.archer_stending_img.copy()
        self.rect = self.image.get_rect()
        self.rect.center = (x, y)
        self.hit_rect = MOB_HIT_RECT.copy()
        self.hit_rect.center = self.rect.center
    ...
    def update(self):
        ...
        if target_dist.length_squared() < DETECT_RADIUS**2:
            ...
            self.image = pg.transform.rotate(
                self.archer_stending_img_0, self.rot-90)
            self.rect.center = self.pos
            self.acc = vec(1, 0).rotate(-self.rot)
            self.avoid_mobs()
            self.avoid_player()
            ...
            self.acc += self.vel * -1
            self.vel += self.acc * self.game.dt
            # if gets too close -> get a distance
    ...
    def draw_health(self):
        if self.health > 60:
            ...
            col = RED
        width = int(self.rect.width * self.health / ARCHER_HEALTH)
        self.health_bar = pg.Rect(0, 0, width, 7)
        if self.health < ARCHER_HEALTH:
            pg.draw.rect(self.image, col, self.health_bar)
